...
def set_recording_key(new_key_code):
    ...
    sql = 'UPDATE User SET recording_key="{0}" WHERE user_id={1}'.format(
        new_key_code, get_user_id()
    )
    cursor.execute(sql)
...
def set_play_mode_key(new_key_code):
    ...
    sql = 'UPDATE User SET play_mode_key="{0}" WHERE user_id={1}'.format(
        new_key_code, get_user_id()
    )
    cursor.execute(sql)
...
def set_current_profile(profile_id):
    ...
    sql = "UPDATE User SET current_profile_id={1} WHERE user_id={0}".format(
        get_user_id(), profile_id
    )
    cursor.execute(sql)
